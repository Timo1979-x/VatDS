...
public class ComboBoxAutoComplete<T> { //extends ComboBox<T> {
    ...
    public ComboBoxAutoComplete(ComboBox<T> cmb) {
        ...
        cmb.getItems().addListener(new ListChangeListener<T>() {
            @Override
            public void onChanged(Change<? extends T> c) {
                ComboBoxAutoComplete.this.originalItems =
                        FXCollections.observableArrayList(cmb.getItems());
            }
        });
        cmb.itemsProperty().addListener(new ChangeListener<ObservableList<T>>() {
            @Override
            public void changed(ObservableValue<? extends ObservableList<T>> observable, ObservableList<T> oldValue, ObservableList<T> newValue) {
                ComboBoxAutoComplete.this.originalItems = FXCollections.observableArrayList(newValue);
            }
        });
    }
